...
public class LoginActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    public void loginFun(final UserClass userClass, final LoginActivity loginActivity, final View view) {
        restaurantApi.loginUser(userClass).enqueue(new Callback<UserClass>() {
            ...
            public void onResponse(Call<UserClass> call, Response<UserClass> response) {
                if (response.isSuccessful()) {
                    ...
                    {
                        SharedPreferences saved_values = PreferenceManager.getDefaultSharedPreferences(getApplicationContext());
                        Editor editor=saved_values.edit();
                    ...
                    if (response.code() ==  422) {
                        try {
                            JSONObject error = (JSONObject) new JSONObject(response.errorBody().string()).get("errors");
                            if (!error.isNull("email")) {
                                JSONArray email = (JSONArray) error.get("email");
                                mEmailView.setError(email.get(0).toString());
                                mEmailView.requestFocus();
                            } if (!error.isNull("password")) {
                                JSONArray password = (JSONArray) ((JSONObject) new JSONObject(response.errorBody().string()).get("errors")).get("password");
                                mPasswordView.setError(password.get(0).toString());
